...
let newDeck () = 
  Random.self_init () ;
  let rec new_aux to_shuffle acc =
    if to_shuffle = [] then acc else
      let i = Random.int (List.length to_shuffle) in
      let c = List.nth to_shuffle i in
      let next = List.filter ((<>) c) to_shuffle in
      new_aux next (c :: acc) in
  new_aux Card.all []
